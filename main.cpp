...
#ifdef EUI_D3D11
    core::d3d::WindowSwapChain swapChain;
#endif
};
...
#ifdef EUI_D3D11
    GLFWwindow* childWindow = glfwCreateWindow(request.width, request.height, request.title.c_str(), nullptr, nullptr);
#else
...
#ifdef EUI_D3D11
    int fbW = 0, fbH = 0;
    glfwGetFramebufferSize(childWindow, &fbW, &fbH);
    HWND childHwnd = glfwGetWin32Window(childWindow);
    managed->swapChain = core::d3d::createSwapChain(childHwnd, fbW, fbH);
    if (!managed->runtime.initialize(childWindow, &managed->swapChain)) {
...
void destroyManagedWindow(std::unique_ptr<ManagedWindow>& managed) {
    ...
    managed->swapChain.rtv.Reset();
    managed->swapChain.swapChain.Reset();
#else
...
#ifdef EUI_D3D11
    if (framebufferWidth != managed.swapChain.width || framebufferHeight != managed.swapChain.height) {
        core::d3d::resizeSwapChain(managed.swapChain, framebufferWidth, framebufferHeight);
    }
#endif
...
#ifdef EUI_D3D11
        managed.runtime.render(framebufferWidth, framebufferHeight, dpiScale, managed.request.clearColor, &managed.swapChain);
        managed.swapChain.swapChain->Present(0, 0);
#else
...
int main() {
    ...
    int mainFbW = 0, mainFbH = 0;
    glfwGetFramebufferSize(window, &mainFbW, &mainFbH);
    HWND mainHwnd = glfwGetWin32Window(window);
    core::d3d::WindowSwapChain mainSwapChain = core::d3d::createSwapChain(mainHwnd, mainFbW, mainFbH);
#else
...
#ifdef EUI_D3D11
    core::d3d::setCurrentSwapChain(&mainSwapChain);
#endif
...
#ifdef EUI_D3D11
            if (framebufferWidth != mainSwapChain.width || framebufferHeight != mainSwapChain.height) {
                core::d3d::resizeSwapChain(mainSwapChain, framebufferWidth, framebufferHeight);
            }
            app::render(framebufferWidth, framebufferHeight, dpiScale);
            mainSwapChain.swapChain->Present(0, 0);
#else
            app::render(framebufferWidth, framebufferHeight, dpiScale);
    ...
    app::shutdown();
#ifdef EUI_D3D11
    mainSwapChain.rtv.Reset();
    mainSwapChain.swapChain.Reset();
#endif
    glfwTerminate();
    return 0;
}
