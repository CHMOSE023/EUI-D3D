...
namespace core {
...
#ifdef EUI_D3D11
        bool createTextureD3D(const unsigned char* pixels, int width, int height,
            ComPtr<ID3D11Texture2D>& outTex, ComPtr<ID3D11ShaderResourceView>& outSrv) {
            ...
            HRESULT hr = device->CreateTexture2D(&desc, &initData, &outTex);
            if (FAILED(hr)) {
                return false;
            }
            ...
            hr = device->CreateShaderResourceView(outTex.Get(), &srvDesc, &outSrv);
            return SUCCEEDED(hr);
        }

        void updateTexturePixelsD3D(ID3D11Texture2D* texture, const unsigned char* pixels, int width, int height) {
            if (texture == nullptr || pixels == nullptr || width <= 0 || height <= 0) {
                return;
            }
            core::d3d::globalCtx().ctx->UpdateSubresource(
                texture, 0, nullptr, pixels, static_cast<UINT>(width) * 4u, 0);
        }
#else
    ...
    bool ImagePrimitive::updateTexture() {
        ...
        int nextHeight = 0;
#ifdef EUI_D3D11
        ID3D11ShaderResourceView* nextSrv = loadTextureD3D(source_, flipVertically_, &pending, &nextWidth, &nextHeight);
        pendingLoad_ = pending;
        ...
        const bool changed = textureSrv_ != nextSrv || loadedSource_ != source_ || loadedFlipVertically_ != flipVertically_;
        releaseOwnedTexture();
        textureSrv_ = nextSrv;
    ...
    void ImagePrimitive::render(int windowWidth, int windowHeight) {
...
#ifdef EUI_D3D11
        if (!initialized_ || textureSrv_ == nullptr) {
            return;
        }
        ...
        core::d3d::uploadVertices(resources.vb.Get(), vertices, sizeof(vertices));

        struct PerFrame { float windowSize[2]; float pad[2]; };
        PerFrame pf{ { static_cast<float>(std::max(1, windowWidth)), static_cast<float>(std::max(1, windowHeight)) }, {} };
        core::d3d::updateConstantBuffer(resources.perFrameCB.Get(), pf);

        struct PerDraw { float tint[4]; float rect[4]; float radius; float pad[3]; };
        PerDraw pd{
            { tint_.r, tint_.g, tint_.b, tint_.a * opacity_ },
            { bounds_.x, bounds_.y, bounds_.width, bounds_.height },
            radius_, {}
        };
        core::d3d::updateConstantBuffer(resources.perDrawCB.Get(), pd);

        constexpr UINT stride = sizeof(float) * 6;
    ...
    bool ImagePrimitive::retainSharedResources() {
        ...
        const char* vsSource =
            ...
            "struct VS_IN { float2 screenPos : POSITION0; float2 localPos : POSITION1; float2 uv : TEXCOORD0; };\n"
            "struct VS_OUT { float4 pos : SV_Position; float2 localPos : TEXCOORD0; float2 uv : TEXCOORD1; };\n"
            "VS_OUT main(VS_IN v) {\n"
            "    VS_OUT o;\n"
        ...
        const char* psSource =
            ...
            "    float4 tint; float4 rect; float radius; float3 _pad;\n"
            "};\n"
            "Texture2D tex : register(t0);\n"
            "SamplerState samp : register(s0);\n"
            "struct PS_IN { float4 pos : SV_Position; float2 localPos : TEXCOORD0; float2 uv : TEXCOORD1; };\n"
            "float roundedBoxDist(float2 p, float2 halfSize, float r) {\n"
            "    float2 c = abs(p) - halfSize + r;\n"
            "    return length(max(c, 0.0)) + min(max(c.x, c.y), 0.0) - r;\n"
            "}\n"
            "float4 main(PS_IN i) : SV_Target {\n"
            "    float2 center = rect.xy + rect.zw * 0.5;\n"
            "    float d = roundedBoxDist(i.localPos - center, rect.zw * 0.5, radius);\n"
            "    float ew = max(abs(ddx(d)) + abs(ddy(d)), 0.75);\n"
            "    float a = 1.0 - smoothstep(-ew, ew, d);\n"
            "    if (a <= 0.0) discard;\n"
            "    float4 s = tex.Sample(samp, i.uv);\n"
            "    return float4(s.rgb * tint.rgb, s.a * tint.a * a);\n"
            "}\n";
        ...
        if (FAILED(hr)) { resources.references = std::max(0, resources.references - 1); return false; }
        ...
        resources.vb = core::d3d::createDynamicVB(sizeof(float) * 36);
        resources.perFrameCB = core::d3d::createConstantBuffer(16);
        resources.perDrawCB = core::d3d::createConstantBuffer(48); // float4 tint + float4 rect + float radius + float3 pad
        return resources.vs && resources.ps && resources.inputLayout && resources.vb && resources.perFrameCB && resources.perDrawCB;
#else
    ...
    bool ImagePrimitive::updateGifTexture(const std::string& resolvedPath) {
        ...
        if (loadedGifPath_ != resolvedPath || loadedGifFlipVertically_ != flipVertically_) {
            ...
            releaseOwnedTexture();
#ifdef EUI_D3D11
            if (!createTextureD3D(gifPixels_.data(), width, height, ownedGifTex_, ownedGifSrv_)) {
                gifPixels_.clear();
                gifDelays_.clear();
                gifFrameCount_ = 0;
                return false;
            }
            textureSrv_ = ownedGifSrv_.Get();
#else
...
#ifdef EUI_D3D11
        if (gifFrameCount_ <= 1 || textureSrv_ == nullptr || gifPixels_.empty()) {
#else
...
#ifdef EUI_D3D11
        updateTexturePixelsD3D(ownedGifTex_.Get(), frame, textureWidth_, textureHeight_);
#else
...
#ifdef EUI_D3D11
    ID3D11ShaderResourceView* ImagePrimitive::loadTextureD3D(const std::string & source, bool flipVertically, bool* pending, int* outWidth, int* outHeight) {
        ...
        if (cached != gTextureCache.end()) {
            if (outWidth)  *outWidth = cached->second.width;
            if (outHeight) *outHeight = cached->second.height;
            return cached->second.srv.Get();
        }
        ...
        rec.width = width;
        rec.height = height;
        const bool ok = createTextureD3D(pixels, width, height, rec.texture, rec.srv);
        if (svgPixels.empty()) {
            stbi_image_free(pixels);
        }
        ...
        ID3D11ShaderResourceView* srv = rec.srv.Get();
        gTextureCache[cacheKey] = std::move(rec);
