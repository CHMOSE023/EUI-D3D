...
namespace core::dsl {
...
#ifdef EUI_D3D11
        bool initialize(GLFWwindow* window, core::d3d::WindowSwapChain* sc) {
            swapChain_ = sc;
            installInputCallbacks(window);
            return true;
        }
...
#ifdef EUI_D3D11
            core::d3d::WindowSwapChain* sc = swapChain_ ? swapChain_ : core::d3d::currentSwapChain();
            renderD3D(windowWidth, windowHeight, dpiScale, clearColor, sc);
#else
            renderGL(windowWidth, windowHeight, dpiScale, clearColor);
#endif
        }

#ifdef EUI_D3D11
        void render(int windowWidth, int windowHeight, float dpiScale, const Color& clearColor,
            core::d3d::WindowSwapChain* sc) {
            swapChain_ = sc;
            renderD3D(windowWidth, windowHeight, dpiScale, clearColor, sc);
        }
#endif
    ...
    private:
        void renderGL(int windowWidth, int windowHeight, float dpiScale, const Color& clearColor) {
#ifndef EUI_D3D11
...
#ifdef EUI_D3D11
        void renderD3D(int windowWidth, int windowHeight, float dpiScale, const Color& clearColor,
            core::d3d::WindowSwapChain* sc) {
            if (!sc || !sc->swapChain) {
                return;
            }
            ...
            if (!ensureRenderCacheD3D(windowWidth, windowHeight)) {
                // No cache: render directly to back buffer
                ComPtr<ID3D11Texture2D> backBuffer;
                sc->swapChain->GetBuffer(0, IID_PPV_ARGS(&backBuffer));
                ComPtr<ID3D11RenderTargetView> backRTV;
                g.device->CreateRenderTargetView(backBuffer.Get(), nullptr, &backRTV);
                ctx->OMSetRenderTargets(1, backRTV.GetAddressOf(), nullptr);
                float cc[4] = { clearColor.r, clearColor.g, clearColor.b, clearColor.a };
                ctx->ClearRenderTargetView(backRTV.Get(), cc);
                setViewportD3D(windowWidth, windowHeight);
            ...
            const std::vector<Rect> dirtyRects = resolveDirtyRects(windowWidth, windowHeight, dpiScale);
            if (dirtyRects.empty() && !fullRedraw_) {
                blitRenderCacheD3D(sc);
                return;
            }

            // Render to cache RTV
            ctx->OMSetRenderTargets(1, cacheRTV_.GetAddressOf(), nullptr);
            setViewportD3D(windowWidth, windowHeight);
            float cc[4] = { clearColor.r, clearColor.g, clearColor.b, clearColor.a };

            // D3D11: ClearRenderTargetView ignores scissor rects, so dirty-rect partial
            // clears would erase the entire cache. Always do a full redraw instead.
            ctx->RSSetState(g.rasterScissorOff.Get());
            ctx->ClearRenderTargetView(cacheRTV_.Get(), cc);
            renderDirect(windowWidth, windowHeight, dpiScale);
    ...
    private:
...
#ifdef EUI_D3D11
            return ensureRenderCacheD3D(width, height);
#else
...
#ifdef EUI_D3D11
        static void applyScissorD3D(const Rect& rect) {
            D3D11_RECT sr;
            sr.left = static_cast<LONG>(std::floor(rect.x));
            sr.top = static_cast<LONG>(std::floor(rect.y));
            sr.right = static_cast<LONG>(std::ceil(rect.x + rect.width));
            sr.bottom = static_cast<LONG>(std::ceil(rect.y + rect.height));
            core::d3d::globalCtx().ctx->RSSetScissorRects(1, &sr);
        }

        static void applyScissor(const Rect& rect, int /*windowHeight*/) {
            applyScissorD3D(rect);
        }

        static void applyOptionalScissor(bool enabled, const Rect& rect, int windowHeight) {
            auto& g = core::d3d::globalCtx();
            if (!enabled) {
                g.ctx->RSSetState(g.rasterScissorOff.Get());
                return;
            }
            applyScissorD3D(rect);
            g.ctx->RSSetState(g.rasterScissorOn.Get());
        }

        static void setViewportD3D(int w, int h) {
            D3D11_VIEWPORT vp{};
            vp.Width = static_cast<float>(w);
            vp.Height = static_cast<float>(h);
            vp.MinDepth = 0.0f;
            vp.MaxDepth = 1.0f;
            core::d3d::globalCtx().ctx->RSSetViewports(1, &vp);
        }

        bool ensureRenderCacheD3D(int width, int height) {
            width = std::max(1, width);
            height = std::max(1, height);
            if (cacheTex_ && cacheWidth_ == width && cacheHeight_ == height) {
                return true;
            }
            releaseRenderCache();
            ...
            if (FAILED(g.device->CreateTexture2D(&desc, nullptr, &cacheTex_))) {
                return false;
            }
            if (FAILED(g.device->CreateRenderTargetView(cacheTex_.Get(), nullptr, &cacheRTV_))) {
                cacheTex_.Reset();
                return false;
            }
            cacheWidth_ = width;
        ...
        void blitRenderCacheD3D(core::d3d::WindowSwapChain* sc) {
            ComPtr<ID3D11Texture2D> backBuffer;
            sc->swapChain->GetBuffer(0, IID_PPV_ARGS(&backBuffer));
            core::d3d::globalCtx().ctx->CopyResource(backBuffer.Get(), cacheTex_.Get());
        }
#else
        static void applyScissor(const Rect& rect, int windowHeight) {
...
#ifdef EUI_D3D11
        ComPtr<ID3D11Texture2D>          cacheTex_;
        ComPtr<ID3D11RenderTargetView>   cacheRTV_;
        core::d3d::WindowSwapChain* swapChain_ = nullptr;
#else
