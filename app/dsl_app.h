...
namespace app {
    ...
    bool initialize(GLFWwindow* window, core::d3d::WindowSwapChain* sc) {
        ...
        return detail::dslRuntime().initialize(window, sc);
    }
#endif
...
#ifdef EUI_D3D11
    void render(int windowWidth, int windowHeight, float dpiScale, core::d3d::WindowSwapChain* sc) {
        if (windowWidth <= 0 || windowHeight <= 0 || dpiScale <= 0.0f) {
            return;
        }
        const core::Color clearColor = dslAppConfig().clearColorValue;
        detail::dslRuntime().render(windowWidth, windowHeight, dpiScale, clearColor, sc);
    }
#endif
